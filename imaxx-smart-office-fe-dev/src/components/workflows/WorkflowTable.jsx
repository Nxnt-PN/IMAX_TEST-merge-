...
export default function WorkflowTable({
  ...
  const columns = [
    ...
    {
      header: t("common:form.workflow-name"),
      accessorKey: "workflow-name",
      cell: ({ row }) => {
        return (
          <div className="workflow-name">{row.original.name}</div>
        );
      },
    },
    {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <>
        <Can required={[PERMISSION.EDIT_WORKFLOW]} permissions={authPermissions} >
          <button
        ...
        <Can required={[PERMISSION.DELETE_WORKFLOW]} permissions={authPermissions} >
          <button
    ...
    <div>
      <div className="search-box mb-4">
        <div className="last-row-filter">
          <div className="">
            <label htmlFor="keyword" className="form-label">{ t("common:form.keyword")}</label>
            <input
              id="keyword"
              className="form-control keyword"
              placeholder={`${t("common:action.search")} ${t("common:form.workflow-name").toLowerCase()}...`}
              type="text"
      ...
      <DataTable data={workflows} columns={columns} loading={loading} />
      <Pagination
        page={query.page}
        limit={query.limit}
        total={total}
